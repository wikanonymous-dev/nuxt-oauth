...
Handler.prototype.createAuth = function createAuth () {
  ...
  return new OAuth2({
    ...
    clientId,
    clientSecret,
    redirectUri: `${protocol}://${this.req.headers.host}/sme/auth/callback`,
    scopes
  })
}
...
Handler.prototype.authenticateCallbackToken = async function authenticateCallbackToken () {
  ...
  try {
    const token = await this.auth.code.getToken(`/sme${this.req.url}`)
...
Handler.prototype.redirectToOAuth = async function redirectToOAuth (redirect) {
  const redirectUrl = `/sme${redirect}` || `/sme${this.req.url}`
...
Handler.routes = {
  login: '/sme/auth/login',
  callback: '/sme/auth/callback',
  logout: '/sme/auth/logout',
...
Handler.prototype.isRoute = function isRoute (route) {
  ...
  return `/sme${this.req.url}`.startsWith(path)
}
